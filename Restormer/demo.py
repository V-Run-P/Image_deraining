...
def get_weights_and_parameters(task, parameters):
    if task == 'Motion_Deblurring':
        ...
        weights = "/home/mdl/shared/afs6372/rain_challenge/Restormer/experiments/Deraining_Restormer/models/net_g_latest.pth" #os.path.join('Deraining', 'pretrained_models', 'deraining.pth')
...
if any([inp_dir.endswith(ext) for ext in extensions]):
    ...
    files = []
    folder_names = set()
    for scene_path in glob(os.path.join(args.input_dir, "*", ""), recursive=True):
        if scene_path.split("/")[-2][:2] >= "Mo":
            folder_names.add(scene_path.split("/")[-2])
            # make output directory 
            os.makedirs(os.path.join(out_dir, scene_path.split("/")[-2]), exist_ok=True)
            # collect all files 
            for ext in extensions:
                files.extend(glob(os.path.join(scene_path, '*.'+ext)))
    files = natsorted(files)

print("CONSIDERED FOLDER NAMES: ", natsorted(list(folder_names)))
if len(files) == 0:
...
with torch.no_grad():
    for file_ in tqdm(files):
        ...
        if task == 'Gaussian_Gray_Denoising':
            ...
            output_file_path = os.path.join(out_dir, file_.split("/")[-2], f+'.png')
            save_img(output_file_path, restored)
